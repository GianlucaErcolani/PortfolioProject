...
#distances
distances = dist(New[,4:5], method = 'euclidean')

hcluster_ward = hclust(distances, method ='ward.D')
plot(hcluster_ward)

hcluster_complete = hclust(distances, method ='complete')
...
#dendograms
plot(hcluster_complete)
rect.hclust(hcluster_complete , k = 5, border = 2:6)

#clusters
ClusterGroups = cutree(hcluster_complete, k=5)

#visualize clusters
fviz_cluster(object = list(data = New,
                           cluster = ClusterGroups),
             choose.vars = c(4, 5),
             geom = "point",
             show.clust.cent = FALSE,
             main = "HC - Complete Linkage") +
